...
> * when plotting, in a time series, the differential between a) funds sent from X to Y and b) funds sent from Y to X, we inadvertently yet consciously point out the *necessary* existence of a 'negative' exchange position of one branch versus the other (either X or Y) and, by extension, a negative exchange position vis-a-vis the branch network. 
